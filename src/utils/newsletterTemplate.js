export function getPremiumNewsletterHTML(issueName, today, campaignData, articlesSource) {
  ...
  const buildPremiumCard = (items) => items.map(a => `
    ...
    <div style="background-color:#ffffff; padding:24px; border-radius:16px; margin-bottom:20px; box-shadow:0 4px 15px rgba(0,0,0,0.03); border:1px solid #f1f5f9; box-sizing:border-box; width:100%;">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:15px;">
        <span style="color:#2563eb; font-size:13px; font-weight:900;">${a.brand || 'OASIS News'}</span>
      ...
      <div style="margin-top:15px; text-align:right;">
        <span style="display:inline-block; color:#94a3b8; font-size:13px; font-weight:bold; text-decoration:underline;">기사 전문 읽기</span>
      </div>
    </div>
    ${linkClose(a.link)}
  `).join('');
  ...
  let html = `
  <div style="background-color:#f4f6f8; padding:24px 0; font-family:'Pretendard','Apple SD Gothic Neo','Malgun Gothic','Segoe UI',sans-serif; width:100%; box-sizing:border-box;">
    ...
    html += `
      <div style="margin-bottom:50px;">
        ...
        <div style="background-color:#ffffff; border-radius:20px; border:1px solid #e2e8f0; overflow:hidden; box-shadow:0 4px 15px rgba(0,0,0,0.03); box-sizing:border-box; width:100%;">
          ${main.img ? `<img src="${main.img}" style="width:100%; max-width:100%; height:auto; border-bottom:1px solid #f1f5f9; display:block;" alt="" />` : ''}
          <div style="padding:25px;">
            <div style="font-size:26px; font-weight:900; color:#0f172a; margin-bottom:15px; line-height:1.4; word-break:keep-all;">${main.isImportant ? '<span style="color:#ef4444; font-size:18px; margin-right:5px;">[HOT]</span>' : ''}${main.title || ''}</div>
            ...
            <div style="margin-top:25px; text-align:center;">
              <span style="display:inline-block; background-color:#0f172a; color:#ffffff; font-size:15px; font-weight:bold; text-decoration:none; padding:14px 28px; border-radius:12px;">기사 전문 읽기 &gt;</span>
            </div>
          </div>
        </div>
        ${linkClose(main.link)}
  ...
  const renderSection = (items, title) => {
    ...
    return `
      <div style="margin-bottom:40px;">
        <div style="font-size:20px; font-weight:900; color:#0f172a; margin-bottom:20px; padding-bottom:10px; border-bottom:2px solid #0f172a;">${title}</div>
  ...
  html += renderSection(security, 'INFO-SECURE');
      ...
      html += `
        <div style="margin-bottom:40px; text-align:left; background:#fdf2f8; border-radius:20px; overflow:hidden; border:1px solid #fbcfe8; box-shadow:0 10px 25px rgba(219,39,119,0.1); box-sizing:border-box;">
          <div style="padding:20px 25px; border-bottom:1px solid #fce7f3;">
            <div style="font-size:16px; font-weight:900; color:#be185d;">추천 영상 ${campaignData.platform || ''}</div>
          </div>
          <div style="padding:24px; box-sizing:border-box;">
            <a href="${safeUrl}" target="_blank" rel="noopener noreferrer" style="display:block; text-decoration:none;">
              <img src="${imgUrl}" style="width:100%; max-width:100%; height:auto; border-radius:12px; display:block; box-shadow:0 4px 10px rgba(0,0,0,0.1);" alt="추천 영상 썸네일">
            </a>
            <div style="padding-top:18px;">
              ...
              <a href="${safeUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block; background:#1e293b; color:white; text-decoration:none; padding:10px 20px; border-radius:8px; font-weight:bold; font-size:14px;">영상 바로보기</a>
            </div>
          </div>
        </div>`;
    } else {
